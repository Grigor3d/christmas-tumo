...
public class OrnamentSpawner : MonoBehaviour
{
    ...
    private List<Ornament> ornamentPrefabs;
    ...
    void Start()
    {
        ornamentPrefabs = new List<Ornament>(Resources.LoadAll<Ornament>("Ornaments"));
    ...
    private void SelectOrnamentPrefab(Ornament ornamentPrefab)
    {
        ...
        if (_selectedObjectToSpawnName == ornamentPrefab.name)
        {
            ...
            // If the position is valid and no ornament is attached, place the ornament
            if (_currentOrnamentPosition != null && string.IsNullOrEmpty(_currentOrnamentPosition.AttachedOrnamentName))
            {
